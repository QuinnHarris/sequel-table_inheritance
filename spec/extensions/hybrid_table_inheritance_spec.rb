...
describe Sequel::Model, "hybrid table inheritance plugin" do
  before do
    ...
    class ::Employee < Sequel::Model(@db)
      ...
      plugin :hybrid_table_inheritance, :key=>:kind, :table_map=>{:Staff=>:staff}, :subclass_load => :eager
    end
  ...
  def should_datasets
    ...
    Cook.dataset.sql.should == "SELECT employees.id, employees.name, employees.kind, staff.manager_id, cooks.speciality FROM employees INNER JOIN staff ON (staff.id = employees.id) INNER JOIN cooks ON (cooks.id = staff.id)"
  end
  ...
  specify "should explicity determine dataset" do
    ...
    Employee.plugin :hybrid_table_inheritance, :key=>:kind, :table_map=>table_map, :subclass_load => :eager
    remove_subclasses
    class ::Unmanaged < Employee; end
    class ::Manager < Employee
      one_to_many :staff_members, :class=>:Staff
  ...
  it "should allow setting and overriding subclass loading options" do
    Employee.dataset.uses_eager_load?.should == true
    Employee.dataset.eager.uses_eager_load?.should == true
    Employee.dataset.lazy.uses_eager_load?.should == false

    Employee.plugin :hybrid_table_inheritance, :subclass_load => :lazy
    Employee.dataset.uses_eager_load?.should == false
    Employee.dataset.lazy.uses_eager_load?.should == false
    Employee.dataset.eager.uses_eager_load?.should == true

    Employee.plugin :hybrid_table_inheritance, :subclass_load => :eager_only
    Employee.dataset.uses_eager_load?.should == true
    Employee.dataset.eager.uses_eager_load?.should == true
    proc{Employee.dataset.lazy.uses_eager_load?}.should raise_error(Sequel::Error)

    Employee.plugin :hybrid_table_inheritance, :subclass_load => :lazy_only
    Employee.dataset.uses_eager_load?.should == false
    Employee.dataset.lazy.uses_eager_load?.should == false
    proc{Employee.dataset.eager.uses_eager_load?}.should raise_error(Sequel::Error)
  end

  it "should initialize subclass datasets" do
    Employee.cti_subclass_datasets.collect { |k, ds| [k, ds.sql] }.should == [
    ...
    Manager.cti_subclass_datasets.collect { |k, ds| [k, ds.sql] }.should == [
    ...
    Staff.cti_subclass_datasets.collect { |k, ds| [k, ds.sql] }.should == [
    ...
    [Unmanaged, SmartManager, GeniusManager, UberManager, DumbManager, Executive].each do |klass|
      klass.cti_subclass_datasets.should == {}
    end
  end
  ...
  it "should eager load subclasses with first" do
    ...
    rec = Employee.first
    rec.class.should == Executive
    rec.values.should == { :id=>1, :kind => 'Executive', :name=>'Tim', :num_managers => 4 }
    @db.sqls.should == [
  ...
  it "should eager load subclasses with all" do
    ...
    list = Employee.all
    list.collect{|x| [x.class, x.values] }.should == [
  ...
  it "should eager load many_to_one relationships correctly" do
    ...
    res = ldr.manager
    res.should == Executive.load(:id=>3, :name=>'E', :kind=>'Executive', :num_managers=>3)
    @db.sqls.should == [
  ...
  it "should eager load one_to_many relationships correctly" do
    ...
    Executive.load(:id=>3).staff_members.should == [Cook.load(:id=>1, :name=>'S', :kind=>'Cook', :manager_id=>3, :speciality=>'Burgers')]
    @db.sqls.should == [
