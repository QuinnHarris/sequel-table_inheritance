# Copied from sequel/spec/extensions/single_table_inheritance_spec.rb
# Minimally modified to test table_inheritance
...
describe Sequel::Model, "single table inheritance plugin" do
  ...
  it "should allow changing the inheritance column via a plugin :single_table_inheritance call" do
    StiTest.plugin :hybrid_table_inheritance, :key=>:blah
    Object.send(:remove_const, :StiTestSub1)
    Object.send(:remove_const, :StiTestSub2)
  ...
  it "should fallback to the main class if the sti_key field is empty or nil without calling constantize" do
    ...
    StiTest.plugin :hybrid_table_inheritance, :key=>:kind
  ...
  # Delete in this plugin only uses the primary key and not the type to select
  it "should destroy the model correctly" do
    ...
    DB.sqls.should == ["DELETE FROM sti_tests WHERE (id = 1)"] * 3
  end
  ...
  it "should handle type column with the same name as existing method names" do
    StiTest.plugin :hybrid_table_inheritance, :key=>:type
  ...
  describe "with custom options" do
    ...
    specify "should have working row_proc if using set_dataset in subclass to remove columns" do
      StiTest2.plugin :hybrid_table_inheritance, :key=>:kind
    ...
    it "should work with custom procs with strings" do
      StiTest2.plugin :hybrid_table_inheritance, :key=>:kind, :model_map=>proc{|v| v == 1 ? 'StiTest3' : 'StiTest4'}, :key_map=>proc{|klass| klass.name == 'StiTest3' ? 1 : 2}
    ...
    it "should work with custom hashes" do
      StiTest2.plugin :hybrid_table_inheritance, :key=>:kind, :model_map=>{0=>StiTest2, 1=>:StiTest3, 2=>'StiTest4'}, :key_map=>{StiTest2=>4, 'StiTest3'=>5, 'StiTest4'=>6}
      class ::StiTest3 < ::StiTest2; end
      class ::StiTest4 < ::StiTest2; end
    ...
    it "should infer key_map from model_map if provided as a hash" do
      StiTest2.plugin :hybrid_table_inheritance, :key=>:kind, :model_map=>{0=>StiTest2, 1=>'StiTest3', 2=>:StiTest4}
      class ::StiTest3 < ::StiTest2; end
      class ::StiTest4 < ::StiTest2; end
      StiTest2.dataset.row_proc.call(:kind=>0).should be_a_instance_of(StiTest2)
      StiTest2.dataset.row_proc.call(:kind=>1).should be_a_instance_of(StiTest3)
    ...
    it "should raise exceptions if a bad model value is used" do
      StiTest2.plugin :hybrid_table_inheritance, :key=>:kind, :model_map=>{0=>1,1=>1.5, 2=>Date.today}
      class ::StiTest3 < ::StiTest2; end
      class ::StiTest4 < ::StiTest2; end
    ...
    it "should work with non-bijective mappings" do
      StiTest2.plugin :hybrid_table_inheritance, :key=>:kind, :model_map=>{0=>'StiTest3', 1=>'StiTest3', 2=>'StiTest4'}
      class ::StiTest3 < ::StiTest2; end
      class ::StiTest4 < ::StiTest2; end
    ...
    # Delete in this plugin only uses the primary key and not the type to select
    it "should destroy the model correctly" do
      StiTest2.plugin :hybrid_table_inheritance, :key=>:kind, :model_map=>{'sti3'=>'StiTest3', 'sti3b'=>'StiTest3', 'sti4'=>'StiTest4'}
      class ::StiTest3 < ::StiTest2; end
      class ::StiTest4 < ::StiTest2; end
      ...
      sqls = DB.sqls
      sqls.should == [
          'SELECT * FROM sti_test3s LIMIT 1',  # Checking for child table
          'SELECT * FROM sti_test4s LIMIT 1',
          'DELETE FROM sti_test2s WHERE (id = 1)',
          'DELETE FROM sti_test2s WHERE (id = 1)',
      ]
    end

    it "should honor a :key_chooser" do
      StiTest2.plugin :hybrid_table_inheritance, :key=>:kind, :key_chooser => proc{|inst| inst.model.to_s.downcase }
      class ::StiTest3 < ::StiTest2; end
      class ::StiTest4 < ::StiTest2; end
